...
export class BackitService {
    ...
    constructor(
        ...
        this.offersParams = {
            ...
            typeId: 1,
            fields: 'id,name,cashbackMaxRate,cashbackRateSymbol,rates,link_match,link_default,image',
            area: 'all',
            limit: 1000,
        }
    }
    ...
    async getOffers(): Promise<Object[]> {
        ...
        return new Promise((resolve, reject) => {
            this.httpService.get(`${this.apiUrl}/offers/list`, config)
                .subscribe(resp => {                    
                    const { status } = resp;
                    if (status === 200) {
                        ...
                        const formatedOffers = offers.map(offer => {
                            ...
                            try {
                                const tempRates = JSON.parse(offer?.attributes?.rates);
                                const ratesArray = tempRates.map(({ newRate, description }) => ({ name: description, value: newRate }));
                                rates = JSON.stringify(ratesArray);
                            } catch (e) {
                                const name = offer?.attributes?.name || 'offer';
                                throw new Error(`Unexpected token in ${name} rates`);
                            }
                            return ({
                                offer: {
                                    ...
                                    rateSymbol: offer?.attributes?.cashbackRateSymbol,
                                },
